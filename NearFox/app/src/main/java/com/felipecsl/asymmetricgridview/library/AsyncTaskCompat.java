 ...
 * and/or handlers.</p>
 *
 * <p>AsyncTaskCompat is designed to be a helper class around {@link Thread} and {@link
 * Handler} and does not constitute a generic threading framework. AsyncTasks should
 ...
 * the <code>java.util.concurrent</code> package such as {@link Executor}, {@link
 * ThreadPoolExecutor} and {@link FutureTask}.</p>
 *
 ...
 *
 * <h2>Usage</h2> <p>AsyncTaskCompat must be subclassed to be used. The subclass will override at
 * least one method ({@link #doInBackground}), and most often will override a second one ({@link
 * #onPostExecute}.)</p>
 *
 * <p>Here is an example of subclassing:</p> <pre class="prettyprint"> private class
 * DownloadFilesTask extends AsyncTaskCompat&lt;URL, Integer, Long&gt; { protected Long
 ...
 *
 * <h2>AsyncTaskCompat's generic types</h2> <p>The three types used by an asynchronous task are the
 ...
 * AsyncTaskCompat&lt;Void, Void, Void&gt; { ... } </pre>
 *
 * <h2>The 4 steps</h2> <p>When an asynchronous task is executed, the task goes through 4 steps:</p>
 * <ol> <li>{@link #onPreExecute()}, invoked on the UI thread before the task is executed. This step
 ...
 * #onProgressUpdate}, invoked on the UI thread after a call to {@link #publishProgress}. The timing
 ...
 * the UI thread after the background computation finishes. The result of the background computation
 ...
 * to work properly:</p> <ul> <li>The AsyncTaskCompat class must be loaded on the UI thread. This is
 * done automatically as of {@link Build.VERSION_CODES#JELLY_BEAN}.</li> <li>The task
 * instance must be created on the UI thread.</li> <li>{@link #execute} must be invoked on the UI
 ...
 *
 * <h2>Memory observability</h2> <p>AsyncTaskCompat guarantees that all callback calls are
 * synchronized in such a way that the following operations are safe without explicit
 * synchronizations.</p> <ul> <li>Set member fields in the constructor or {@link #onPreExecute}, and
 ...
 * single background thread. Starting with {@link Build.VERSION_CODES#DONUT}, this was
 * changed to a pool of threads allowing multiple tasks to operate in parallel. Starting with {@link
 * Build.VERSION_CODES#HONEYCOMB}, tasks are executed on a single thread to avoid common
 * application errors caused by parallel execution.</p> <p>If you truly want parallel execution, you
 * can invoke {@link #executeOnExecutor(Executor, Object[])} with {@link
...
public abstract class AsyncTaskCompat<Params, Progress, Result> {
   ...
   * serially on a single background thread. Starting with {@link Build.VERSION_CODES#DONUT},
   * this was changed to a pool of threads allowing multiple tasks to operate in parallel. Starting
   * {@link Build.VERSION_CODES#HONEYCOMB}, tasks are back to being executed on a single
   ...
   *                               Status#RUNNING} or {@link Status#FINISHED}.
   ...
   * caller can keep a reference to it.
   *
   * <p>Note: this function schedules the task to run serially with another tasks using the {@link
   * #SERIAL_EXECUTOR};. * <p>This method must be invoked on the UI thread.
   *
   * @param params The parameters of the task.
   * @return This instance of AsyncTaskCompat.
   * @throws IllegalStateException If {@link #getStatus()} returns either {@link
   ...
   * caller can keep a reference to it.
   *
   * <p>Note: this function schedules the task to run in parallel with another tasks using the
   ...
   * to guarantee such work is serialized regardless of platform version you can use the {@link
   * #execute} version of this method
   *
   * <p>This method must be invoked on the UI thread.
   *
   * @param params The parameters of the task.
